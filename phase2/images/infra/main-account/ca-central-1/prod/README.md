...
Orchestrated by the [Makefile](../../../../Makefile) (`make images-infra-*`, `make images-config-build`). The Makefile invokes [`images/setup-images.sh`](../../../../setup-images.sh).
...
| Same workspace for destroy | Run `make images-infra-destroy` (or `./images/setup-images.sh destroy`) from the **same directory and state file** that performed the apply. Otherwise Terraform may plan to recreate resources or miss ones still running in AWS. |
...
Consider S3 (+ DynamoDB lock table) if you need shared applies, CI-driven deploy/destroy, or state that survives disk loss. The cluster live root (`cluster/infra/main-account/ca-central-1/prod/`) will likely need remote state sooner because it is long-lived.
...
| `phase2/cluster/infra/main-account/ca-central-1/prod/` | Cluster live root (separate state) |
